import torch
from torch import nn
from models.triplet_loss import HardTripletLoss
import torch.nn.functional as F
import numpy as np
from timm.loss import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy


# 各种分布特征聚合
class LossFun(nn.Module):
    def __init__(self, args):
        super(LossFun, self).__init__()
        self.mse_loss = nn.MSELoss()
        # self.mse_loss = nn.L1Loss()
        self.triplet_loss = HardTripletLoss(margin=args.margin, hardest=True)
        self.ce_loss = LabelSmoothingCrossEntropy(smoothing=0.1).cuda()
        self.alpha = args.alpha_tl
        self.alpha_ce = args.alpha_ce
        self.alpha_mse = args.alpha_mse

    def forward(self, pred, label, feat, feat2, clip, clip2, args):
        # feat (b, n, c), x (b, t, c)
        if feat is not None:
            device = feat.device
            b, n, c = feat.shape
            flat_feat = feat.view(-1, c)  # (bn, c)
            la = torch.arange(n, device=device).repeat(b)

            t_loss = self.triplet_loss(flat_feat, la)
            # t_loss = pair_diversity_loss(feat)
            if feat2 is not None:
                device = feat2.device
                b, n, c = feat2.shape
                flat_feat2 = feat2.view(-1, c)  # (bn, c)
                la2 = torch.arange(n, device=device).repeat(b)

                t_loss += self.triplet_loss(flat_feat2, la2)
        else:
            self.alpha = 0
            t_loss = 0
        if clip is not None:
            device = clip.device
            b, n, c = clip.shape
            flat_feat = clip.view(-1, c)  # (bn, c)
            la = torch.arange(n, device=device).repeat(b)

            ce_loss = self.ce_loss(flat_feat, la)
            if clip2 is not None:
                device = clip2.device
                b, n, c = clip2.shape
                flat_feat2 = clip2.view(-1, c)  # (bn, c)
                la2 = torch.arange(n, device=device).repeat(b)

                ce_loss += self.ce_loss(flat_feat2, la2)
        else:
            self.alpha_ce = 0
            ce_loss = 0
        mse_loss = self.mse_loss(pred, label)
        # mse_loss = pearson_loss(pred, label)
        return self.alpha_mse * mse_loss + self.alpha * t_loss + self.alpha_ce * ce_loss, mse_loss, t_loss, ce_loss
        # return f_loss, mse_loss, t_loss, f_loss
